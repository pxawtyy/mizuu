...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !Maria.authState.creds.registered) {
      if (useMobile) throw new Error('não foi possível utilizar o código de pareamento com api móvel.')
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("comece com o código de país do seu número, exemplo: +5511911223193")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`o número do seu bot do WhatsApp\nexemplo: +5511911223193`)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`🤖 seu código: `)), chalk.black(chalk.white(code)))
      }, 3000)
   }
...
Maria.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
console.log(chalk.green('🟨 bem vindo ao ✠ 𝚖ͫ✞᪸ͥ𝚣ᷦ𝚞𐨿ͧ𝚞ͧ𑀇᷼'));
...
MariaLft = await getBuffer(ppuser)
		...
		if (!global.welcome) return;	
                ...
                Mariabody = `┌──⊰ 🎗 𝙗𝙚𝙢-𝙫𝙞𝙣𝙙𝙤(𝙖) 🎗⊰
│⊳  🌐 ᵖᵃʳᵃ: ${metadata.subject}
│⊳  📋 ⁿᵒᵐᵉ: @${MariaName.split("@")[0]}
│⊳  👥 ᵐᵉᵐᵇʳᵒˢ: ${xmembers}
│⊳  🕰️ ᵉⁿᵗʳᵒᵘ: ${xtime} ${xdate}
└──────────⊰
`
...
"sourceUrl": `${link}`}}})
                	...
                	const Mariabuffer = await getBuffer(ppuser)
                    const Mariatime = moment.tz('America/Sao_Paulo').format('HH:mm:ss')
	                const Mariadate = moment.tz('America/Sao_Paulo').format('DD/MM/YYYY')
                	let MariaName = num
                    const Mariamembers = metadata.participants.length
  Mariabody = `┌──⊰🍁 𝙖𝙙𝙚𝙪𝙨 🍁⊰
│⊳  👤 ᵈᵉ: ${metadata.subject}
│⊳  📔 ⁿᵒᵐᵉ: @${MariaName.split("@")[0]}
│⊳  👥 ᵐᵉᵐᵇʳᵒˢ: ${Mariamembers}
│⊳  🕒 ʰᵒʳᵃ: ${Mariatime} ${Mariadate}
└──────────⊰
...
const Mariadate = moment.tz('America/Sao_Paulo').format('DD/MM/YYYY')
let MariaName = num
Mariabody = `𝙥𝙖𝙧𝙖𝙗𝙚́𝙣𝙨! 🎉 \n@${MariaName.split("@")[0]}, você foi *promovido(a)* para *administrador!*`
   Maria.sendMessage(anu.id,
 { text: Mariabody,
 contextInfo:{
 mentionedJid:[num],
...
const Mariadate = moment.tz('America/Sao_Paulo').format('DD/MM/YYYY')
let MariaName = num
Mariabody = `𝗼𝗽𝘀‼️ \n@${MariaName.split("@")[0]}, você foi *rebaixado* para *membro...*`
Maria.sendMessage(anu.id,
 { text: Mariabody,
 contextInfo:{
 mentionedJid:[num],
